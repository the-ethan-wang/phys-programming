...
def strings(weights: list[Union[float,int]], net_force: Optional[Union[float,int]], net_acceleration: Optional[Union[float,int]]):
    """Returns list of every force acting on each object. Assumes force is applied from the side of the weights, and the strings are taut."""
    ...
    for i, weight in enumerate(weights):
        ...
        if i==0:
            if net_force<0:
                ...
                obj_data["forces"].append(["Tension", obj_net_force-net_force])
            else:
                obj_data["forces"].append(["Tension", obj_net_force])
            ...
            obj_data["forces"].append(["Tension", -1*ans['forces'][i-1]['forces'][-1][1]])
            obj_data["forces"].append(["Tension", obj_net_force-obj_data['forces'][0][1]])
        ans['forces'].append(obj_data)
    return ans

examples=[
    {"name": "Question 1", "weights": [15, 25], "net force": 160},
    {"name": "Question 2", "weights": [8, 4], "net force": -96},
    {"name": "Question 3", "weights": [14, 6, 30], "net force": -150},
    {"name": "Question 4", "weights": [12, 6, 8], "net force": 156},
    {"name": "Question 5", "weights": [4, 3, 12, 6], "net force": 6.25},
]

for example in examples:
    weights=example["weights"]
    net_force=example.get("net force")
    net_acceleration=example.get("net acceleration")
    data=strings(weights=weights, net_force=net_force, net_acceleration=net_acceleration)
